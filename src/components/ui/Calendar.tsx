...
function Calendar({
    ...
    const _weekdayClassName = cn(
        "w-8 text-base font-normal text-muted-foreground",
    ...
    const _dayClassName = cn(
        "flex w-8 h-8 flex-1 items-center justify-center p-0 text-base",
        props.dayClassName
    );
    const _dayButtonClassName = cn(
        buttonVariants({ variant: "ghost" }),
        "w-8 h-8 rounded-md p-0 font-normal transition-none aria-selected:opacity-100",
        ...
        <DayPicker
            ...
            formatters={{
                formatCaption: (month, options, dateLib) => {
                    const date = dateLib!.format(month, "MMMM yyyy", {
                        locale: th,
                    });
...
function CaptionLabel({
    ...
    return (
        <Button
            className="h-7 w-full truncate text-base font-medium"
            variant="ghost"
            size="sm"
            onClick={() =>
        ...
        >
            {navView === "days"
                ? children
                : displayYears.from + 543 + " - " + (displayYears.to + 543)}
        </Button>
    );
}
                ...
                (_, i) => {
                    ...
                    return (
                        <Button
                            key={i}
                            className={cn(
                                "h-7 w-full text-base font-normal text-foreground",
                                displayYears.from + i ===
